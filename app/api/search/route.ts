  ...
  try {
    if (!q) {
      ...
      const results = await Promise.all(randomMembers.map(async (member) => {
        const [warnsRes, timeoutsRes, bansRes, streaksRes, coinsRes] = await Promise.allSettled([
          ...
          query(`SELECT * FROM coins WHERE user_id = $1`, [member.id])
        ]);
        ...
        const coinsData = coinsRes.status === 'fulfilled' && coinsRes.value.rows.length > 0 ? coinsRes.value.rows[0] : { tasks_remaining: {}, tasks_completed: {} };
        ...
        return {
          ...
          stats: {
            ...
            tasks_remaining: Object.keys(coinsData.tasks_remaining || {}).length,
            tasks_completed: Object.keys(coinsData.tasks_completed || {}).length,
          }
        };
      }));
    ...
    if (discordInfo) {
      const [warnsRes, timeoutsRes, bansRes, streaksRes, coinsRes] = await Promise.allSettled([
        ...
        query(`SELECT * FROM coins WHERE user_id = $1`, [q])
      ]);
