...
export function TopUsersSection({ guildId, initialData }: { guildId: string, initialData?: any }) {
  ...
  useEffect(() => {
    // If we already have data from the server, don't fetch immediately
  ...
  const sections = [
    { key: 'topDay', title: 'الأكثر تفاعلاً لليوم', icon: Flame, color: 'text-orange-400', bg: 'bg-orange-500/10', border: 'border-orange-500/30', glow: 'rgba(249,115,22,0.15)' },
    { key: 'topWeek', title: 'الأكثر تفاعلاً للأسبوع', icon: Star, color: 'text-yellow-400', bg: 'bg-yellow-500/10', border: 'border-yellow-500/30', glow: 'rgba(234,179,8,0.15)' },
    { key: 'topOverall', title: 'الأكثر تفاعلاً بالسيرفر', icon: Crown, color: 'text-purple-400', bg: 'bg-purple-500/10', border: 'border-purple-500/30', glow: 'rgba(168,85,247,0.15)' }
  ];
    ...
    return (
      <section className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200 fill-mode-both">
        <div className="flex items-center gap-4 px-2">
          ...
          <h2 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-l from-white to-purple-200 tracking-tight drop-shadow-sm">أكثر الأعضاء تفاعلاً</h2>
        </div>
    ...
    return (
      <section className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200 fill-mode-both">
        ...
        <div className="flex justify-center py-20 text-gray-400 bg-[#0a0f1a]/80 backdrop-blur-2xl rounded-[2rem] border border-white/5 shadow-[0_8px_30px_rgb(0,0,0,0.5)] font-medium text-lg">
          لا يوجد أعضاء متفاعلين حالياً أو جاري تحديث البيانات...
        </div>
      </section>
    );
  }
